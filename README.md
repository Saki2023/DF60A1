...
DNF 国服 2008（客户端 `1.0.1.9`）本地模拟端研究项目。模拟端使用 `.NET Framework 4.8.1 / x86` 和 SQLite；客户端协议通过静态 IDA 分析及本机实测复原，未修改 `DNF.exe` 二进制。

本仓库只包含模拟端源码，不包含 `DNF.exe`、`Script.pvf`、IDA 数据库、NPK、音乐或其他客户端资源。

## 放置目录和 PVF

请将整个 `DF60A1` 仓库目录放到 DNF 客户端根目录下，并自行把与客户端匹配的 `Script.pvf` 放进仓库的 `data` 目录：
...
模拟端默认从 `DF60A1/data/Script.pvf` 读取频道脚本，从仓库上一级目录查找 `DNF.exe`。缺少 PVF 时服务端会明确报错并退出。
...
例如在仓库根目录直接启动上一级目录的客户端：

```powershell
Start-Process -FilePath '..\DNF.exe' -WorkingDirectory '..' -ArgumentList '99?127.0.0.1?7001?create_channel_2008?test?0?0?0?0?0?0?0?0'
```
...
IDA 中的 `sub_40BE30` 使用 `?` 分隔原始启动数据。少于 13 段时设置调试标志；调试标志打开时，`DebugConfig.txt` 存在就调用 `sub_4086F0` 载入离线角色，不存在则读取资源字符串 101 并显示从 `start\DNFchina.exe` 启动的错误。

当第 1 段为 `99` 时，第 2、3 段直接写入服务器 IP 和端口；第 4、5 段是账号和密码，其余字段目前使用兼容占位值。

### 创建角色页面

`DNF.exe` 1.0.1.9 的角色选择/创建模块 RTTI 为 `CNEntranceModule`：
...
早期命令号和部分角色/城镇包布局参考了 55 级模拟端及 `df_game_r.i64` 的符号流程，但服务端下行变换、频道布局和最终字段均以 2008 国服 `DNF.exe` 1.0.1.9 的静态分析及实测为准。入口实测依次发送协议 11、5、9、1；`sub_894B10` 确认频道元数据为 `组数 + 组名[20] + 频道数 + 频道记录`。客户端字符串表的 `china_90` 映射内部组号 99，对应启动参数首段与 PVF 中的测试服组 99。未知包会保留完整十六进制日志，后续逐项确认。
